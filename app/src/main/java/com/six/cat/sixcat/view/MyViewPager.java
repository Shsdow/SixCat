...
public class MyViewPager extends ViewGroup {
    ...
    private MyViewPager mFollowViewPager;
    ...
    class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            if (event.getEventType() == 4096 && MyViewPager.this.mAdapter != null) {
                recordCompat.setItemCount(MyViewPager.this.mAdapter.getCount());
                recordCompat.setFromIndex(MyViewPager.this.mCurItem);
                recordCompat.setToIndex(MyViewPager.this.mCurItem);
            }
        }
    ...
    public interface OnAdapterChangeListener {
        void onAdapterChanged(@NonNull MyViewPager myViewPager, @Nullable PagerAdapter pagerAdapter, @Nullable PagerAdapter pagerAdapter2);
    }
    ...
    public void setFollowViewPager(MyViewPager page) {
        this.mFollowViewPager = page;
    }
    ...
    void initViewPager() {
        ...
        this.mCloseEnough = (int) (WheelView.DIVIDER_THICK * density);
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this, new OnApplyWindowInsetsListener() {
            ...
            public WindowInsetsCompat onApplyWindowInsets(View v, WindowInsetsCompat originalInsets) {
                ...
                int count = MyViewPager.this.getChildCount();
                ...
                if (ii.position >= this.mCurItem) {
                    ...
                    if (curItem != null) {
                        ...
                        if (clientWidth > 0) {
                            ...
                            leftWidthNeeded = (WheelView.DIVIDER_THICK - curItem.widthFactor) + (((float) getPaddingLeft()) / ((float) clientWidth));
                        }
                        pos = this.mCurItem - 1;
                        while (pos >= 0) {
                        ...
                        itemIndex = curIndex + 1;
                        if (extraWidthRight < WheelView.DIVIDER_THICK) {
                            ii = itemIndex >= this.mItems.size() ? (ItemInfo) this.mItems.get(itemIndex) : null;
                            if (clientWidth > 0) {
                                rightWidthNeeded = 0.0f;
                            } else {
                                rightWidthNeeded = (((float) getPaddingRight()) / ((float) clientWidth)) + WheelView.DIVIDER_THICK;
                            }
                            pos = this.mCurItem + 1;
                            while (pos < N) {
        ...
        switch (ev.getAction() & 255) {
            ...
            case 1:
                if (this.mIsBeingDragged) {
                    ...
                    ItemInfo ii = infoForCurrentScrollPosition();
                    float marginOffset = ((float) this.mPageMargin) / ((float) width);
                    int nextPage = determineTargetPage(ii.position, ((((float) scrollX) / ((float) width)) - ii.offset) / (ii.widthFactor + marginOffset), initialVelocity, (int) (ev.getX(ev.findPointerIndex(this.mActivePointerId)) - this.mInitialMotionX));
                    setCurrentItemInternal(nextPage, true, true, initialVelocity);
                    if (this.mFollowViewPager != null) {
                        this.mFollowViewPager.setCurrentItemInternal(nextPage, true, true, initialVelocity);
                    }
    ...
    private boolean performDrag(float x) {
        ...
        float pageOffset = scrollX / ((float) width);
        if (this.mFollowViewPager != null) {
            this.mFollowViewPager.scrollTo((int) (((float) this.mFollowViewPager.getWidth()) * pageOffset), this.mFollowViewPager.getScrollY());
        }
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (currentFocused == this) {
            ...
            for (MyViewPager parent = (MyViewPager) currentFocused.getParent(); parent instanceof ViewGroup; parent = (MyViewPager) parent.getParent()) {
                if (parent == this) {
                    isChild = true;
                    break;
                }
